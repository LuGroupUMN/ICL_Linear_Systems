...
#     plt.loglog(n_ls[:-1], mse_mat[i, :-1] - mse_mat[i, -1], '--*', label=f'm={m_ls[i]}')
# plt.loglog(n_ls[:-1], 1e-3 * n_ls[:-1]**(-1.0), 'r', label=r'$O(N^{-0.5})$')
mse_mat = mse_mat[:, 3:]
n_ls = n_ls[3:]
# print(mse_mat.shape)
# zxc
...
scale_1p5 = ref_y / (ref_N ** (-1.5))
scale_1 = ref_y / (ref_N ** (-1.0))

plt.loglog(n_ls[:-3], 1e-1 * scale_1 * n_ls[:-3]**(-1.0), 'b', label=r'$O(n^{-2})$')
