...
h1_mat = h1_vec.reshape(len(n_ls), len(m_ls))
...
mse_ref_rate = 3e-1 * scale_1 * m_ls**(-1.0)
...
scale_1 = ref_y / (ref_m ** (-1.0))
h1_ref_rate = 3e-1 * scale_1 * m_ls**(-0.5*2)

h1_mat = h1_mat[:, :-2] - h1_mat[:, [-1]]

npy_name = 'Elliptic_Result_test_m.npy'
...
# plt.loglog(m_ls[:-2], 1e-1 * m_ls[:-2]**(-1.0), 'r', label=r'$O(m^{-1})$')
#
...
# plt.savefig(f'test_m_mse_l1_{num2str_decimal(l1)}_l2_{num2str_decimal(l2)}_Ltype_{Ltype}_Vtype_{Vtype}.png', bbox_inches = 'tight')
#
#
# plt.figure(2)
# for i in range(len(n_ls)):
#     plt.loglog(m_ls[:-2], h1_mat[i, :-2] - h1_mat[i, -1], '--*', label=f'n={n_ls[i]}')
# plt.loglog(m_ls[:-2], 1e-1 * m_ls[:-2]**(-1.0), 'r', label=r'$O(m^{-1})$')
#
# #plt.title(f'd={d},N={N}', fontsize=18)
# plt.xlabel(r'$m$', fontsize=18)
...
# plt.yticks(fontsize=12)
# plt.legend(fontsize=12)
# plt.savefig(f'test_m_h1_l1_{num2str_decimal(l1)}_l2_{num2str_decimal(l2)}_Ltype_{Ltype}_Vtype_{Vtype}.png', bbox_inches = 'tight')
#
#
# plt.show()
