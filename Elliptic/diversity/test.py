...
# plt.figure(1)
# plt.loglog(m_ls, l2_mat[0, ...], label=r'$V(x)=c, c \sim U(1,2)$')
# for i in range(1, len(para_ls)):
#     # plt.loglog(m_ls, l2_mat[i, ...], '--', label=rf'$(\alpha_1,\beta_1)=$(' + str(para_ls[0][0])+','+str(para_ls[0][1])+')')
#     plt.loglog(m_ls, l2_mat[i, ...], '--',
#                label=rf'$V(x) \sim \mathbf{{U}} {para_ls[i][0], para_ls[i][1]}$')
#
...
# plt.savefig(f'div_op_1_l1_{num2str_decimal(l1)}_l2_{num2str_decimal(l2)}_Ltype_{Ltype}_Vtype_{Vtype}.png', bbox_inches = 'tight')
# plt.show()
...
npy_name = 'Elliptic_Result_diversity_test.npy'
