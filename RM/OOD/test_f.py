...
l2_mat = l2_vec.reshape(len(para_f), len(mul_ls))

h1_vec = np.array(h1_ls)
h1_mat = h1_vec.reshape(len(para_f), len(mul_ls))
...
# plt.figure(1)
# plt.loglog(m_ls, l2_mat[0, ...], label=rf'$\rho={para_f[0]}$')
# for i in range(1, len(para_f)):
#     plt.loglog(m_ls, l2_mat[i, ...], '--', label=rf'$\rho={para_f[i]}$')
#
...
# plt.savefig(f'test_f.png', bbox_inches = 'tight')
# plt.show()
